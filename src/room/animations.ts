 ...
 */
export function useIntroAnimation(refs: RefObject<IntroRefs>, revealed: boolean) {
  useEffect(() => {
    if (!revealed) return;

    const timelines = TRACKS.map(({ timeScale = 1, delay, steps }) => {
      const tl = gsap.timeline({ defaults: { duration: DURATION, ease: EASE, delay } });
      tl.timeScale(timeScale);
      for (const { key, position, delay: stepDelay } of steps) {
        const scale = refs.current[key]?.scale;
        if (scale) tl.to(scale, { x: 1, y: 1, z: 1, delay: stepDelay }, position);
      }
      return tl;
    });
